...
namespace FancyToys.Nursery {
    ...
    public class ProcessManager {
        ...
        public ProcessInfo Add(string pathName) {
            ...
            Dogger.Info($"Add {Path.GetFileName(pathName)} succeed.");
        ...
        public ProcessInfo Launch(int pid) {
            Dogger.Trace($"Launch {pid}");

            // no process via such `pid`
            if (!Processes.TryGetValue(pid, out ProcessInfo pi)) {
                Dogger.Error($"Process {pid} doesn't exist.");
                return null;
            }

            // process is already running
            if (pi.IsRunning) {
                Dogger.Warn($"Process {pi.Alias} is running");
                return null;
            }
            ...
            lock (_launchLock) {
                ...
                if (!launchSucceed) { // launch failed
                    Dogger.Error($"Process launch failed: {pi.Alias}");
                    return null;
                }
            }
        ...
        public ProcessInfo Stop(int pid) {
            if (!Processes.TryGetValue(pid, out ProcessInfo pi)) {
                Dogger.Error($"No such process with PID: {pid}");
            ...
            if (!ps.HasExited) {
                ...
                Dogger.Warn($"Process {pi.Alias}({pi.Id}) already exited.");
            }
        ...
        public ProcessInfo Remove(int pid) {
            ...
            Dogger.Trace($"Process removed: {pid}, {pi.Alias}");
            return pi;
        }
        ...
        public ProcessInfo(int id, Process ps, string alias, ProcessManager pm, ProcessManager.ProcessInfoHandler processExited) {
            ...
            Pcs.Exited += (sender, _) => {
                ...
                if (AutoRestart) {
                    ...
                    Dogger.Info($"Process {alias}, running:{IsRunning} exited.");
                }
            };
